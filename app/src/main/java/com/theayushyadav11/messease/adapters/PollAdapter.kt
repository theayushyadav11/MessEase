...
class PollsAdapter(private val pollsList: MutableList<Poll>, private val context: Context) :
    ...
    fun deleteItem(position: Int, uid: String) {
        ...
        FirebaseDatabase.getInstance().reference.child("polls").child(pollsList[position].uid)
            .removeValue()
        FirebaseDatabase.getInstance().reference.child("pollResult").child(pollsList[position].uid)
            .removeValue()
